...
| API full run 2 | `npm run test --workspace @fool/api` | PASS 1014/1014, 0 skipped |
...
| Design validation | `npm run validate:design` | PASS |
| Reference DDL | `npm run validate:ddl` | PASS; PostgreSQL 17.10, 50 tables twice |
| Secret scan | `npm run validate:preparation:security` | PASS; 0 findings |
| Validator failure paths | `npm run validate:preparation:failure-paths` | PASS; 9/9 expected failures detected |
