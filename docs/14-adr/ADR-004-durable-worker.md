---
id: EF-ADR-004
title: ADR-004：独立 Durable Worker
version: '1.1'
status: accepted
...
HTTP 内 Apply 在请求/进程中断时丢失活动状态，无法安全恢复副作用。

## 决策

所有长执行通过 ExecutionRun、PostgreSQL Queue、Worker Lease、Heartbeat、Fencing 和 Checkpoint。
...
同步 HTTP；只用内存队列；无 fencing 的定时扫描。

## 后果

增加调度和恢复复杂度，但为 Dataset/Cutover/Archive 提供必要基础。
