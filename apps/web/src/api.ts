...
export interface Phase2Run { id:string; workspaceId:string; projectId:string; runType:string; operationClass:string; planHash:string; approvalHash:string; state:string; version:number; createdAt:string; startedAt?:string; completedAt?:string }
export async function listPhase2Runs(token:string,workspaceId:string,projectId?:string):Promise<Phase2Run[]>{const query=projectId?`?projectId=${encodeURIComponent(projectId)}`:"";const response=await fetch(`/api/v1/runs${query}`,{headers:phase1Headers(token,workspaceId)});const body=await readJsonOrThrow<{items:Phase2Run[]}>(response,"Load durable Runs failed");return body.items;}
export async function commandPhase2Run(token:string,run:Phase2Run,command:"pause"|"resume"|"cancel",reason:string):Promise<Phase2Run>{const response=await fetch(`/api/v1/runs/${encodeURIComponent(run.id)}/${command}`,{method:"POST",headers:phase1Headers(token,run.workspaceId,true,{"If-Match":`"${run.version}"`}),body:JSON.stringify({reason})});return readJsonOrThrow(response,`Run ${command} failed`);}
