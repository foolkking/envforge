...
test("failed production migration rolls back its schema and version record", async () => {
  ...
  await fs.copyFile(resolveFromRoot("apps/api/migrations/postgres", "0004_phase2_durable_execution.sql"), path.join(migrationRoot, "0004_phase2_durable_execution.sql"));
  await fs.writeFile(path.join(migrationRoot, "0005_failure_probe.sql"), "CREATE TABLE platform.must_rollback(id integer); SELECT missing_column FROM platform.must_rollback;\n");
  ...
  const version = await database.pool.query("SELECT 1 FROM platform.schema_migrations WHERE version='0005'");
...
test("PostgreSQL backup restores into an empty disposable database", async () => {
  ...
  assert.deepEqual((await restored.migrate()).map((item) => item.version), ["0001", "0002", "0003", "0004"]);
