...
export interface StoredProbeSnapshot {
  agentId: string;
  collectedAt: string;
  /** Overall evidence quality. Optional for snapshots written before the envelope was introduced. */
  collection?: {
    status: "ok" | "partial" | "failed";
    completeness: number;
    commands: Array<{ command: string; exitCode?: number; timedOut?: boolean }>;
    stderr?: string;
    errors: string[];
    timedOut: boolean;
  };
  /** Per-section evidence prevents collector failure from being interpreted as an empty finding. */
...
export interface RuntimeDatabase {
  ...
  /** Advisory user decisions remembered by the Decision Engine. */
  decisionUserPreferences?: import("./decision-engine/user-preferences.js").UserDecisionPreference[];
  /** Scoped assignments of built-in Decision Engine risk profiles. */
  decisionProfileAssignments?: import("./decision-engine/profiles.js").DecisionProfileAssignment[];
  /** Persistent human review queue generated by Decision Engine evaluations. */
  decisionReviewInbox?: import("./decision-engine/review-inbox.js").ReviewInboxItem[];
  /** Bounded append-only Decision Engine evaluation history. */
  decisionHistory?: import("./decision-engine/history.js").DecisionHistoryRecord[];
  /** Bounded append-only audit trail for Decision Engine lifecycle events. */
  decisionAuditLog?: import("./decision-engine/audit.js").DecisionAuditRecord[];
  ...
  /**
   * Apply run claims for immutable Environment Plans.
   * Created atomically before a non-dry-run apply enters managed execution.
   */
  applyRuns?: StoredApplyRun[];
  /**
   * Idempotency-key index for non-dry-run Environment Plan Apply.
   * Scope is userId + planId + planHash + key.
   */
  applyIdempotencyRecords?: ApplyIdempotencyRecord[];
  /**
...
export interface StoredSchedule {
  ...
  /** Display name */
  name: string;
  /** Immutable Environment Plan identity. Schedules never execute raw playbooks/catalog entries. */
  planId?: string;
  approvedPlanHash?: string;
  /** @deprecated Legacy records are retained for migration but are never executed. */
  playbookId?: string;
  /** @deprecated Legacy records are retained for migration but are never executed. */
  catalogId?: string;
...
function normalizeRuntimeDatabase(database: Partial<RuntimeDatabase>): RuntimeDatabase {
  ...
  return {
    ...
    decisionUserPreferences: database.decisionUserPreferences ?? [],
    decisionProfileAssignments: database.decisionProfileAssignments ?? [],
    decisionReviewInbox: database.decisionReviewInbox ?? [],
    decisionHistory: database.decisionHistory ?? [],
    decisionAuditLog: database.decisionAuditLog ?? [],
    ...
    applyRuns: database.applyRuns ?? [],
    applyIdempotencyRecords: database.applyIdempotencyRecords ?? [],
...
export interface StoredApplyRun {
  id: string;
  userId: string;
  planId: string;
  planHash: string;
...
export interface ApplyIdempotencyRecord {
  key: string;
  userId: string;
  planId: string;
  planHash: string;
...
export interface StoredEnvironmentPlan {
  ...
  payload: unknown;
  /** Approval is a runtime record bound to the immutable plan hash. */
  approvalRecord?: import("./environment-plan.js").PlanApprovalRecord;
  /** Active non-dry-run apply claim, if managed execution is in progress. */
  activeApplyRunId?: string;
  activeApplyPlanHash?: string;
  lastDryRunAt?: string;
  lastDryRunResult?: {
    ok: boolean;
    planHash: string;
